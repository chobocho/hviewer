...
static void scroll_to_line(int line);   /* bookmark.h의 cmd_bookmark_jump가 호출 */
...
/* 도메인별 헤더-온리 모듈 — g_state 정의 이후 위치.
 * 단일 TU 빌드 모델 유지 (build.bat 무변경). */
#include "prompt.h"          /* PromptCtx, prompt_input */
#include "search_history.h"  /* search_history_load/save/add/rebuild_menu */
#include "recent.h"          /* recent_load/save/add/remove/rebuild_menu, cmd_clear_recent */
#include "bookmark.h"        /* bookmark_index_of/has, bookmarks_load/save, cmd_bookmark_* */
...
#include "help_dialog.h"   /* HelpCtx, cmd_show_help (g_state.hwnd 의존) */
